...
6. **循循善诱** — 帮助用户发现可能遗漏的场景，如异常流程、权限控制、数据一致性、并发场景。
...
- 优先先进行自然讨论与总结，`ask_user` 用于关键信息缺失时的精准补问与收口确认
- 不要在开场直接问卷式抛出多题；仅在必要时调用 `ask_user`
- **永远直接调用工具提问**：决定提问时，直接调用 `ask_user`。不要先用文字说"我来问几个问题"然后停住
...
def build_project_context(
    ...
    if _perms and "ask_user" in _perms and not _code_tool_perms:
        ...
        ask_only_notice = (
            ...
            "- 先自然讨论并总结，再在关键信息缺失时使用 `ask_user` 精准补问。\n"
            "- 不要尝试调用 read_file、search_text、get_file_tree 等工具——它们不可用。\n"
            "- 不要求每次回复都调用 `ask_user`；仅在必要时调用。\n"
    ...
    if role:
        ...
        if getattr(role, "name", "") == "需求分析":
            named_parts.append((
                "需求讨论会话风格（高优先级覆盖）",
                """以下规则优先于历史旧规则：
- 先与用户自然讨论并复述理解，不要开场问卷式连发问题。
- 提问应少量、精准、客观（每轮优先 1-2 个关键问题）。
- 避免主观偏好型开放问题；优先可决策、可落地的问题。
- 临近敲定时，再对遗漏与关键信息做补问收口（范围、约束、验收、异常流程）。
"""
            ))
        if getattr(role, "name", "") == "Bug 问诊":
            named_parts.append((
                "Bug 问诊会话风格（高优先级覆盖）",
                """以下规则优先于历史旧规则：
- 先引导用户连续描述现象与复现过程，再复述确认理解。
- 不要开场问卷式连发；仅在必要时用 ask_user 精准补问（每轮 1-2 个）。
- 问题应客观、可验证、可执行；避免主观偏好型开放问题。
- 临近输出诊断书前，再集中补齐遗漏信息（环境、复现稳定性、影响范围）。
"""
            ))
    else:
